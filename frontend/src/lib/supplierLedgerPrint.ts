...
import { summarizeSupplierLedger } from "@/lib/supplierLedgerUtils";
import {
...
export interface SupplierLedgerPrintMeta {
  supplierCode: string;
  supplierName: string;
  fromDate?: string;
  toDate?: string;
...
function buildLedgerPrintHtml(
  ...
  const periodLine = meta.useDateFilter && meta.fromDate && meta.toDate
  ...
  const rows = entries
    ...
    <tr>
      ...
      <td class="mono sm">${escapeHtml(row.transactionCode)}</td>
...
<head>
  <meta charset="utf-8" />
  <title>Supplier ledger — ${escapeHtml(meta.supplierName)}</title>
  <style>
    * { box-sizing: border-box; }
    ...
    .supplier { margin-top: 6px; font-size: 12px; font-weight: 600; }
    ...
    td.sm { font-size: 8px; color: #64748b; }
...
<body>
  <div class="header">
    ...
    <p class="title">Supplier account ledger</p>
    <p class="supplier">${escapeHtml(meta.supplierCode)} — ${escapeHtml(meta.supplierName)}</p>
  ...
  <table>
    ...
    <tbody>
      ${rows || '<tr><td colspan="9" style="text-align:center;padding:16px;color:#64748b">No entries</td></tr>'}
    </tbody>
    <tfoot>
      <tr>
        <td colspan="6" style="text-align:right;text-transform:uppercase;font-size:8px;color:#64748b">Period totals</td>
  ...
  <p class="footer">Supplier ledger statement · ${escapeHtml(meta.supplierName)} · ${escapeHtml(branding.storeName)}</p>
...
/** Print supplier ledger (iframe, opens system print dialog). */
export function printSupplierLedgerReport(
  entries: SupplierLedgerEntry[],
  meta: SupplierLedgerPrintMeta
): void {
  const html = buildLedgerPrintHtml(entries, meta);
  printHtmlDocument(html, `Supplier ledger — ${meta.supplierName}`);
}
